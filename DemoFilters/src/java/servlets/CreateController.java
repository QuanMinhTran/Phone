    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(!id.trim().isEmpty() && !id.matches("[0-9]{2}")){
                errorObj.setIdError("Computer ID must be numerical, 2 digits");
                valid = false;
            }
            if(cpu.trim().isEmpty()){
                errorObj.setIdError("CPU is not supposed to be empty");
                valid = false;
            }
            if(!cpu.trim().isEmpty() && cpu.length() < 6){
                errorObj.setIdError("CPU information of computer is greater than 6 characters");
                valid = false;
            }
            if(vga.trim().isEmpty()){
                errorObj.setIdError("VGA is not supposed to be empty");
                valid = false;
            }
            if(ram.trim().isEmpty()){
                errorObj.setIdError("RAM is not supposed to be empty");
                valid = false;
            }
            if(hardDisk.trim().isEmpty()){
                errorObj.setIdError("Hard Disk is not supposed to be empty");
                valid = false;
            }
            if(monitor.trim().isEmpty()){
                errorObj.setIdError("Monitor is not supposed to be empty");
                valid = false;
            }
